  ...
  describe "GET #show" do
    context "given invalid emails" do
      it "redirects to index with error" do
        ...
        get :show, params: { id: nil , email: "mistressofallevil@example.org" }
        it_redirects_to_with_error(invite_requests_path, message)
        expect(assigns(:invite_request)).to be_nil
      end

      it "renders for an ajax call" do
        get :show, params: { id: nil }, xhr: true
        expect(response).to render_template("show")
        expect(assigns(:invite_request)).to be_nil
        get :show, params: { id: nil, email: "mistressofallevil@example.org" }, xhr: true
        expect(response).to render_template("show")
        expect(assigns(:invite_request)).to be_nil
      end
    end
    ...
    context "given valid emails" do
      ...
      it "renders" do
        get :show, params: { id: nil, email: invite_request.email }
        expect(response).to render_template("show")
        expect(assigns(:invite_request)).to eq(invite_request)
      end

      it "renders for an ajax call" do
        get :show, params: { id: nil, email: invite_request.email }, xhr: true
        expect(response).to render_template("show")
        expect(assigns(:invite_request)).to eq(invite_request)
      end
    end
